...
def smoker_callback(ch, method, properties, body):
    ...
    try: 
        ...
        fooda_mess = body.decode().split(",")
        # check to see if message contains a valid temperature to add to deque
        if fooda_mess[1] != "No temperature":
            fooda_temp = float(fooda_mess[1])
            fooda_timestamp = fooda_mess[0]
            fooda_deque.append(fooda_temp)
            
        if len(fooda_deque) == 20:
            foodaalert = fooda_deque[0]-fooda_deque[19]
            if foodaalert < 1:
                print(alert)
